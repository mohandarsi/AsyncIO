...
OverlappedIOController::Disable()
{
	for (Threads::const_iterator it = m_dispatcherThreads.begin(), end = m_dispatcherThreads.end(); it != end; ++it)
	{
		...
		if (0 == ::PostQueuedCompletionStatus(m_hCompletionPort, 0, CK_STOP, 0))
		{
...
{
    if (!CreateIoCompletionPort(fileHandle.m_hFileHandle, m_hCompletionPort,
    ...
    if (fileHandle.m_hFileWriteHandle != INVALID_HANDLE_VALUE &&
        !CreateIoCompletionPort(fileHandle.m_hFileHandle, m_hCompletionPort,
                                reinterpret_cast<ULONG_PTR>(&fileHandle), 0)) {
...
OverlappedIOController::DispatcherThread()
{
    ...
    while (true) {
		...
		BOOL ok = ::GetQueuedCompletionStatus(m_hCompletionPort, &bytes_transferred,
